...
const api = axios.create({
    baseURL: import.meta.env.VITE_API_URL  //BaseURL: serve para definir um enreço base fixo
})
...
export const buscar = async (url: string, setDados: Function, header: Object) => {
    const resposta = await api.get(url, header) //Ele faz uma requisição HTTP do tipo GET para a URL especificada, enviando os dados para o servidor passando o token que fica no header.
    setDados(resposta.data)
...
export const cadastrar = async (url: string, dados: Object, setDados: Function, header: Object) => {
    const resposta = await api.post(url, dados, header) // aqui faz uma requisição do metodo POST enviando a url, os dados e o token de autenticação
    setDados(resposta.data)
}
